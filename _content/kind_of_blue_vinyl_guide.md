...
A tape machine error on March 2 ran the prime three-track deck roughly 2.4% slow (confirmed by spectrum analysis), causing Side 1 to play sharp on every pressing made from that tape – both the original mono and stereo. The defect went undiscovered for 33 years until Mark Wilder found it in 1992; the original mono masters were already lost by then, so all modern "mono" reissues are new dedicated mono remixes from the three-track safety reel – an active mixing process that handles the discrete center channel (carrying Davis's trumpet) separately, not a simple passive fold-down of two channels into one. This single fact divides the entire pressing landscape into two lineages: the **Grundman/Classic/AP** chain (cut direct from the three-track session master in 1995, one generation closer to the source) and the **Wilder/Sony/MoFi** chain (from Wilder's 1997 two-track fold-down). Every serious pressing decision follows from understanding which lineage you are buying into.
